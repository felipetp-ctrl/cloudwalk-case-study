...
def join_labels(requests: pd.DataFrame, labels: pd.DataFrame) -> pd.DataFrame:
    ...
    if not ip_labels.empty:
        ...
        merged = merged[(merged.timestamp >= merged.active_from) & (merged.timestamp <= merged.active_until)]
        if not merged.empty:
    ...
    if all_matches:
        ...
        requests = requests.merge(deduped[['request_id', 'attack_class', 'confidence']],
